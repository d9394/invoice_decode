...
def pdf_decode(pfile):
	pdf = pdfplumber.open(pfile)
#	print("PDF页数: %s" % len(pdf.pages) )
	pdf_text = pdf.pages[0].extract_text()
	pdf.close()
	print("PDF第一页：%s" % pdf_text)
#	pdb.set_trace()  # 设置断点
	fapiaolexing = re_text(re.compile(r'.*(专用发票|普通发票).*'), pdf_text)
	fapiaodaima = re_text(re.compile(r'(发票代码|票据代码)(.*\d+)'), pdf_text)
	fapiaohaoma = re_text(re.compile(r'(发票号码|票据号码)(.*\d+)'), pdf_text)
	...
	heji = re.split("¥|￥",re_text(re.compile(r'合\s+计(.*)'), pdf_text))
	try :
		tax = heji[2]
	except :
		tax = 0
	...
	if company:
		...
		gongsi = ""
	#xixiang = re.findall(re.compile(r'\*.*\*.* '), pdf_text)
	xixiang = re.findall(re.compile(r'([^\d\W]\w*)\*([^\d\W]\w*)'), pdf_text)
	if xixiang :
		#xiangmu = ",".join([re_block(item.split(' ')[0]) for item in xixiang])
